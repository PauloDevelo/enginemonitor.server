...
describe('Images', () => {
  ...
  beforeEach(async () => {
    ...
    roUser = new Users({
      name: 'read', firstname: 'only', email: 'read.only@gmail.com', forbidUploadingImage: true,
    });
  ...
  describe('/GET/:parentUiId images', () => {
    it('it should GET a 200 http code as a result because images were returned successfully', async () => {
      ...
      let image1 = new Images({
        _uiId: 'image_01', description: 'image 1 description', name: 'image1', parentUiId: 'engine_01', path: image1Path, thumbnailPath: thumbnail1Path, title: 'image1',
      });
      ...
      let image2 = new Images({
        _uiId: 'image_02', description: 'image 2 description', name: 'image2', parentUiId: 'engine_01', path: image2Path, thumbnailPath: thumbnail2Path, title: 'image2',
      });
      ...
      let image3 = new Images({
        _uiId: 'image_03', description: 'image 3 description', name: 'image3', parentUiId: 'engine_01', path: image3Path, thumbnailPath: thumbnail3Path, title: 'image3',
      });
      image3 = await image3.save();

      // Act
      const res = await chai.request(app).get(`/api/images/${engine._uiId.toString()}`).set('Authorization', `Token ${user.generateJWT()}`);
    ...
    it('it should GET a 200 http code as a result for the read only user', async () => {
      ...
      // Act
      const res = await chai.request(app).get(`/api/images/${engine._uiId.toString()}`).set('Authorization', roUserJWT);
    ...
    it('it should GET a 400 http code as a result because the user does not exist', async () => {
      ...
      let image1 = new Images({
        _uiId: 'image_01', description: 'image 1 description', name: 'image1', parentUiId: 'engine_01', path: imagePath, thumbnailPath, title: 'image1',
      });
      image1 = await image1.save();

      // Act
      const res = await chai.request(app).get(`/api/images/${engine._uiId.toString()}`).set('Authorization', `Token ${fakeUser.generateJWT()}`);
  ...
  describe('/POST/:parentUiId new image', () => {
    it('it should GET a 200 http code as a result because the image was added successfully', async () => {
      ...
      res.body.image.thumbnailUrl.should.startsWith(`http://localhost:8000/api/uploads/${user._id.toString()}`);
      res.body.image.thumbnailUrl.should.endsWith(`thumbnail_${engine._uiId}.jpeg`);

      res.body.image.url.should.startsWith(`http://localhost:8000/api/uploads/${user._id.toString()}`);
      res.body.image.url.should.endsWith(`${engine._uiId}.jpeg`);
    ...
    it('it should get a 500 http code error as a result because the user storage is full', async () => {
      ...
      expect(fs.readdirSync(config.get('ImageFolder') + user._id.toString()).length).to.be.equal(successfulAttempts * 2);
    });
    ...
    it('it should GET a 422 http code as a result because the _uiId was missing', async () => {
      ...
      // Act
      const res = await chai.request(app).post(`/api/images/${engine._uiId}`)
        .field('name', 'my first image added')
        .field('parentUiId', engine._uiId)
        .attach('imageData', fs.readFileSync('tests/toUpload/image4.jpeg'), `${engine._uiId}.jpeg`)
        .attach('thumbnail', fs.readFileSync('tests/toUpload/thumbnail4.jpeg'), `thumbnail_${engine._uiId}.jpeg`)
  ...
  describe('/POST/:parentUiId/:imageUiId change an image', () => {
    it('it should get a 200 http code as a result because the image name changed successfully', async () => {
      ...
      const jsonImage = { name: 'image1 modified' };

      // Act
      const res = await chai.request(app).post(`/api/images/${engine._uiId}/${jsonImage1._uiId}`).send({ image: jsonImage }).set('Authorization', `Token ${user.generateJWT()}`);
    ...
    it('it should get a 400 http code and a credential error as a result because the read only user does not have enough credential to edit the image', async () => {
      ...
      // Act
      const res = await chai.request(app).post(`/api/images/${engine._uiId}/${jsonImage1._uiId}`).send({ image: jsonImage }).set('Authorization', roUserJWT);
    ...
    it('it should get a 200 http code as a result because the image description changed successfully', async () => {
      ...
      const jsonImage = { description: 'image1 description modified' };
    ...
    it('it should get a 400 http code as a result because the image does not belong to the boat of the request', async () => {
      ...
      let watermaker = new Equipments({
        name: 'WaterMakert', brand: 'Katadin', model: 'Powersurvivor40', age: 1234, installation: '2018/01/20', _uiId: 'equipment_02',
      });
      ...
      const thumbnail1Path = `${config.get('ImageFolder')}thumbnail1.jpeg`;
      let image1 = new Images({
        _uiId: 'image_01', description: 'image 1 description', name: 'image1', parentUiId: watermaker._uiId, path: image1Path, thumbnailPath: thumbnail1Path, title: 'image1',
      });
      image1 = (await image1.save());
      const jsonImage1 = await image1.toJSON();
    ...
    it('it should GET a 400 http code as a result because the user does not exist', async () => {
      ...
      const thumbnail1Path = `${config.get('ImageFolder')}thumbnail1.jpeg`;
      let image1 = new Images({
        _uiId: 'image_01', description: 'image 1 description', name: 'image1', parentUiId: engine._uiId, path: image1Path, thumbnailPath: thumbnail1Path, title: 'image1',
      });
      image1 = (await image1.save());
      const jsonImage1 = await image1.toJSON();
      ...
      // Act
      const res = await chai.request(app).post(`/api/images/${engine._uiId}/${jsonImage1._uiId}`).send({ image: jsonImage }).set('Authorization', `Token ${fakeUser.generateJWT()}`);
  ...
  describe('/DELETE/:parentUiId/:imageUiId delete image', () => {
    it('it should get a 200 http code as a result because the orphan image was deleted successfully', async () => {
      ...
      const thumbnail1Path = `${config.get('ImageFolder')}thumbnail1.jpeg`;
      let image1 = new Images({
        _uiId: 'image_01', description: 'image 1 description', name: 'image1', parentUiId: 'watermaker_01', path: image1Path, thumbnailPath: thumbnail1Path, title: 'image1',
      });
      image1 = (await image1.save());

      // Act
      const res = await chai.request(app).delete(`/api/images/watermaker_01/${image1._uiId}`).set('Authorization', `Token ${user.generateJWT()}`);
    ...
    it('it should get a 200 http code as a result because the image was deleted successfully', async () => {
      ...
      // Act
      const res = await chai.request(app).delete(`/api/images/${engine._uiId}/${image1._uiId}`).set('Authorization', `Token ${user.generateJWT()}`);
    ...
    it('it should get a 400 http code and a credential error because the read only user does not have credential', async () => {
      ...
      // Act
      const res = await chai.request(app).delete(`/api/images/${engine._uiId}/${image1._uiId}`).set('Authorization', roUserJWT);
