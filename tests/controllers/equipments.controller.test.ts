...
describe('Equipments', () => {
  ...
  describe('/GET equipments', () => {
    ...
    beforeEach(async () => {
      equipment = new Equipments({
        name: 'Arbutus', brand: 'Nanni', model: 'N3.30', age: 1234, installation: '2018/01/20', _uiId: 'boat_01',
      });
    ...
    it('it should GET a 401 http code as a result because the request does not have the token', async () => {
      // Act
      const res = await chai.request(app).get(`/api/equipments/${boat._uiId}`);
    ...
    it('it should get a 400 http code as a result because the token is invalid', async () => {
      ...
      // Act
      const res = await chai.request(app).get(`/api/equipments/${boat._uiId}`).set('Authorization', userJWT);
    ...
    it('it should get a 401 http code as a result because the token is expired', async () => {
      ...
      // Act
      const res = await chai.request(app).get(`/api/equipments/${boat._uiId}`).set('Authorization', expiredToken);
    ...
    it('it should GET a 200 http code as a result and a equipment because we set a read only user token', async () => {
      ...
      // Act
      const res = await chai.request(app).get(`/api/equipments/${boat._uiId}`).set('Authorization', roUserJWT);
  ...
  describe('POST equipments', () => {
    it('it should get a 401 http code as a result because the request does not have the token', async () => {
      // Act
      const res = await chai.request(app).post(`/api/equipments/${boat._uiId}`);
    ...
    it('it should get a 400 http code as a result because the token is invalid', async () => {
      ...
      const equipment = {
        name: 'Arbutus', brand: 'Nanni', model: 'N3.30', age: 1234, installation: '2018-01-09T23:00:00.000Z', _uiId: 'boat_01',
      };

      // Act
      const res = await chai.request(app).post(`/api/equipments/${boat._uiId}`).send({ equipment }).set('Authorization', userJWT);
    ...
    it('it should get a 200 http code as a result because the equipment was successfully created', async () => {
      // Arrange
      const equipment = {
        name: 'Arbutus', brand: 'Nanni', model: 'N3.30', age: 1234, installation: '2018-01-09T23:00:00.000Z', ageAcquisitionType: AgeAcquisitionType.manualEntry, ageUrl: '', _uiId: '123456',
      };

      // Act
      const res = await chai.request(app).post(`/api/equipments/${boat._uiId}/${equipment._uiId}`).send({ equipment }).set('Authorization', userJWT);
    ...
    it('it should get a 400 http code credential error as a result because the user is readonly', async () => {
      ...
      // Act
      const res = await chai.request(app).post(`/api/equipments/${boat._uiId}/${equipment._uiId}`).send({ equipment }).set('Authorization', roUserJWT);
    ...
    it('it should get a 422 http error code because there is already an equipment with the same name', async () => {
      // Arrange
      const equipment = {
        name: 'Arbutus', brand: 'Nanni', model: 'N3.30', age: 1234, installation: '2018-01-09T23:00:00.000Z', ageAcquisitionType: AgeAcquisitionType.manualEntry, ageUrl: '', _uiId: 'fvvaqrtkmint',
      };
      let res = await chai.request(app).post(`/api/equipments/${boat._uiId}/${equipment._uiId}`).send({ equipment }).set('Authorization', userJWT);

      const equipment2 = {
        name: 'Arbutus', brand: 'Nanni', model: 'N3.30', age: 1234, installation: '2018-01-09T23:00:00.000Z', ageAcquisitionType: AgeAcquisitionType.manualEntry, ageUrl: '', _uiId: 'asdfdfas',
      };
  ...
  describe('POST/:equipmentId equipment', () => {
    ...
    beforeEach(async () => {
      engine = new Equipments({
        name: 'Arbutus', brand: 'Nanni', model: 'N3.30', age: 1234, installation: '2018/01/20', _uiId: 'boat_01', assetId: boat._id,
      });
    ...
    it('it should get a 200 http code as a result because the equipment age was successfully modified', async () => {
      ...
      // Act
      const res = await chai.request(app).post(`/api/equipments/${boat._uiId}/${engine._uiId}`).send(modifications).set('Authorization', userJWT);
    ...
    it('it should get a 400 http code credential error as a result because the user is readonly', async () => {
      ...
      // Act
      const res = await chai.request(app).post(`/api/equipments/${boat._uiId}/${engine._uiId}`).send(modifications).set('Authorization', roUserJWT);
  ...
  describe('DELETE/:equipmentId equipment', () => {
    ...
    it('it should get a 200 http code as a result because the equipment was successfully deleted', async () => {
      ...
      let entry3 = new Entries({
        name: 'My third entry', date: new Date().toString(), age: 12347, remarks: 'RAS3', _uiId: 'entry-02',
      });
    ...
    it('it should get a 400 http code credential error because the user is read only', async () => {
      ...
      // Act
      const res = await chai.request(app).delete(`/api/equipments/${boat._uiId}/${engine._uiId}`).set('Authorization', roUserJWT);
    ...
    it('it should get a 400 http code as a result because the equipment does not exist', async () => {
      // Act
      const res = await chai.request(app).delete(`/api/equipments/${boat._uiId}/5c27912a9bc7e61fdcd2e82c`).set('Authorization', userJWT);
    ...
    it('it should get a 400 http code as a result because the equipment requested is not own by the user', async () => {
      ...
      // Act
      const res = await chai.request(app).delete(`/api/equipments/${boat._uiId}/${engine._uiId}`).set('Authorization', `Token ${anotherUser.generateJWT()}`);
